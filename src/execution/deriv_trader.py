...
class DerivTradeExecutor:
    ...
    def get_maxhold_exit_info(self, symbol: str) -> dict[str, Any]:
        """D.9.0: True si el último trade cerró por max_hold LIMPIO (sin spike adentro).

        Condición: exit_reason empieza con 'max_hold' Y peak_profit == 0.0.
        Si peak_profit > 0 hubo un spike dentro de la posición → no re-entrar.
        """
        last = self._last_closed_sym.get(symbol.upper())
        if last is None:
